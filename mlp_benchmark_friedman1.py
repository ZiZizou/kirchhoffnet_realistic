"""Minimal MLP baseline for the Friedman #1 synthetic regression task.

Purpose: provide a parameter-matched benchmark for the reduced differential
KirchhoffNet on the classic Friedman (1991) MARS test problem #1.

Friedman #1 is a 10-dimensional regression problem. Only x1..x5 carry
signal; x6..x10 are uniformly distributed dummy noise variables that
should be ignored by a learning algorithm. The target is:

    y = 10 * sin(pi * x1 * x2)
      + 20 * (x3 - 0.5)^2
      + 10 * x4
      +  5 * x5
      + noise,   noise ~ N(0, sigma^2)

with all xi ~ U(0, 1) and the canonical sigma = 1.0.

CLI:
    mlp_benchmark_friedman1.py [--hidden-dim 100] [--num-layers 2] [--epochs 800]
...
__all__ = ["MLPRegressor", "count_parameters", "_friedman1", "make_data_friedman1"]


_FRIEDMAN1_IN_DIM = 10
_FRIEDMAN1_RELEVANT = 5
_FRIEDMAN1_PI = math.pi
...
def main():
    parser = argparse.ArgumentParser(
        description="Train an N-layer MLP on the Friedman #1 synthetic regression task."
    )
    ...
    with open(out_dir / "config_snapshot.txt", "w") as f:
        f.write(f"model: MLPRegressor(10 -> {args.hidden_dim} -> 1, "
        ...
        if do_validate:
            if val_loss < best_val - args.min_delta:
                ...
                if epochs_without_improve >= args.patience:
                    print(
                        f"[mlp_friedman1] early stopping at epoch {epoch}: "
    ...
    print(f"[mlp_friedman1] training done in {elapsed:.1f}s ({len(history)} epochs)")

    if best_state is not None:
        net.load_state_dict(best_state)
        print(f"[mlp_friedman1] restored best model from epoch {best_epoch} (val={best_val:.6f})")
    ...
    plot_output_fit(
        ...
        title=f"MLP (hidden={args.hidden_dim}, layers={args.num_layers}, {args.activation}) — Friedman #1 output fit",
    )
    ...
    if args.noise or args.noise_aware:
        print(
            f"[mlp_friedman1] running MC noise eval: quant_bits={args.quant_bits} "
        ...
        print(
            f"[mlp_friedman1] noise eval: clean={clean_loss:.6f} "
    ...
    print(f"[mlp_friedman1] artifacts in {out_dir}")
