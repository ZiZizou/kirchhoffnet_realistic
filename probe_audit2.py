...
# --- 12. Twin probe API surface (plan twin-node-tanh, spec twin-probe-spec)
for name in (
...
# Convention: tap k at row t holds u[t-k] (causal, zero-padded for t<k).
...
torch.manual_seed(0)
u_dl, _ = tiny_stream(n=200)
u_scaled = ne._scale_drive(u_dl, bipolar=True, order=10, input_scale=1.0)
drive1 = nlc._twin_drive(u_scaled, "scalar", n_taps=8, input_scale=1.0)
drive2 = nlc._twin_drive(u_scaled, "scalar", n_taps=8, input_scale=1.0)
...
# --- 16. Twin Jacobian analytic form vs autograd -------------------------
# The analytic Jacobian is the per-step Jacobian at (x, d).  Compare
# against an autograd Jacobian of the equivalent single-step F(x) =
# (1-a)*x + a*tanh(W*x + W_in*d).  _twin_run() detaches per-step state
# so it is NOT a valid autograd reference; we re-construct the local map
# here to verify the closed-form.
torch.manual_seed(0)
...
J_analytic = nlc._twin_jacobian_at(
    W=W, W_in=W_in, x=x, drive_row=d, a=a_twin,
)
...
# twin_node_tanh() must NOT attach _lin_* attributes to the module, must
# NOT touch fabric stage/cell_lib state, and must return a valid report.
...
new_module_attrs = set(nlc.__dict__.keys()) - before_keys
