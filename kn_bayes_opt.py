...
def main() -> None:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        if args.dataset == "ctle":
            ...
            # ── CTLE preflight: --count-params-only gate (mirrors train_script path) ──
            # Rejects over-budget configs before paying the 4×100 cost. Uses the
            # same --count-params-only mechanism as the non-CTLE branch so the
            # sampler receives a finite graded penalty and actually learns.
