"""Minimal MLP baseline for the Friedman #3 synthetic regression task.

Purpose: provide a parameter-matched benchmark for the reduced differential
KirchhoffNet on the classic Friedman (1991) MARS test problem #3.

Friedman #3 is a 4-dimensional regression problem. The target is:

    y = arctan((x2 * x3 - 1 / (x2 * x4)) / x1) + noise
...
CLI:
    mlp_benchmark_friedman3.py [--hidden-dim 100] [--num-layers 2] [--epochs 800]
...
__all__ = ["MLPRegressor", "count_parameters", "_friedman3", "make_data_friedman3"]


_FRIEDMAN3_IN_DIM = 4
_FRIEDMAN3_PI = math.pi
_FRIEDMAN3_RANGES = [
    (0.0, 100.0),
    (40.0 * _FRIEDMAN3_PI, 560.0 * _FRIEDMAN3_PI),
    (0.0, 1.0),
    (1.0, 11.0),
]
...
def make_data_friedman3(batch_size: int, noise_std: float = 1.0, val_size: int = 4000):
    """Build Friedman #3 train/val loaders with target standardization.
...
def main():
    parser = argparse.ArgumentParser(
        description="Train an N-layer MLP on the Friedman #3 synthetic regression task."
    )
    ...
    parser.add_argument("--target-noise-std", type=float, default=1.0,
                        help="Standard deviation of additive Gaussian target "
                             "noise (default: 1.0, the Friedman #3 canonical value).")
    ...
    parser.add_argument("--output", type=Path, default=Path("./output/mlp_friedman3"),
                        help="Output directory (default: ./output/mlp_friedman3)")
    ...
    train_loader, val_loader, task_fn, inverse_stats = make_data_friedman3(
    ...
    print(
        f"[mlp_friedman3] hidden_dim={args.hidden_dim} num_layers={args.num_layers} "
    ...
    with open(out_dir / "config_snapshot.txt", "w") as f:
        ...
        f.write(f"dataset: Friedman #3 (4-dim, ranges x1=(0,100) x2=(40π,560π) "
        ...
        if do_validate:
            if val_loss < best_val - args.min_delta:
                ...
                if epochs_without_improve >= args.patience:
                    print(
                        f"[mlp_friedman3] early stopping at epoch {epoch}: "
    ...
    print(f"[mlp_friedman3] training done in {elapsed:.1f}s ({len(history)} epochs)")

    if best_state is not None:
        net.load_state_dict(best_state)
        print(f"[mlp_friedman3] restored best model from epoch {best_epoch} (val={best_val:.6f})")
    ...
    plot_loss_curve(
        ...
        title=f"MLP (hidden={args.hidden_dim}, layers={args.num_layers}, {args.activation}, {n_params} params) — Friedman #3",
    ...
    plot_output_fit(
        ...
        title=f"MLP (hidden={args.hidden_dim}, layers={args.num_layers}, {args.activation}) — Friedman #3 output fit",
    )
    ...
    print(
        f"[mlp_friedman3] final val {loss_label} = {full_val_loss:.6f} "
        f"(best val = {best_val:.6f} @ epoch {best_epoch})  "
    ...
    if args.noise or args.noise_aware:
        print(
            f"[mlp_friedman3] running MC noise eval: quant_bits={args.quant_bits} "
        ...
        print(
            f"[mlp_friedman3] noise eval: clean={clean_loss:.6f} "
    ...
    print(f"[mlp_friedman3] artifacts in {out_dir}")
