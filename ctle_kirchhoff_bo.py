...
class KirchhoffCircuit(nn.Module):
    """The original repository's bare Circuit RHS, with CPU/GPU-safe buffers."""
    def __init__(self, src: torch.Tensor, dst: torch.Tensor, activation: str) -> None:
        super().__init__()
        self.register_buffer("src", src.long())
        self.register_buffer("dst", dst.long())
        self.num_nodes = int(torch.maximum(src, dst).max().item())  # excludes ground node 0
    ...
    def forward(self, _t: torch.Tensor, voltage: torch.Tensor) -> torch.Tensor:
        # Ground is fixed to zero. Each directed device is i=f(a*v_src+b*v_dst+c).
        with_ground = torch.cat((torch.zeros_like(voltage[:, :1]), voltage), dim=1)
...
class CTLEKirchhoffNet(nn.Module):
    ...
    def __init__(self, nodes: int, edge_count: int, activation: str, topology_seed: int,
                 t_end: float, rtol: float) -> None:
        super().__init__()
        src, dst = make_topology(nodes, edge_count, topology_seed)
        self.encoder = nn.Linear(4, nodes)
        self.circuit = KirchhoffCircuit(src, dst, activation)
    ...
    def forward(self, specs: torch.Tensor) -> torch.Tensor:
        initial_voltage = self.encoder(specs)
        ...
        return self.readout(final_voltage)
...
def parameter_count(nodes: int, edges: int) -> int:
    # encoder (4N+N bias), three parameters/device, readout (7N+7 bias)
    return 12 * nodes + 3 * edges + 7
...
def trial_train(trial: optuna.Trial, data: dict[str, Any], args: argparse.Namespace, device: torch.device,
    ...
    model = CTLEKirchhoffNet(nodes, edges, activation, args.seed + trial.number, t_end, rtol).to(device)
    ...
    torch.save({"state_dict": state, "nodes": nodes, "edges": edges, "activation": activation,
                "t_end": t_end, "rtol": rtol, "validation_loss": best, "parameter_count": parameter_count(nodes, edges)},
...
def run(args: argparse.Namespace) -> None:
    ...
    (output / "run_config.json").write_text(json.dumps({**vars(args), "fingerprint": data["fingerprint"], "candidate_count": len(candidates), "solver": "odeint_adjoint/dopri5"}, indent=2, default=str))
    ...
    model = CTLEKirchhoffNet(payload["nodes"], payload["edges"], payload["activation"], args.seed + best.number, payload["t_end"], payload["rtol"]).to(device)
